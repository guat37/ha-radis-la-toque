...
class RadisLaToqueConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Configure one restaurant fiche."""
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            try:
                restaurants = await self.client.async_find_restaurants(
                    str(user_input[CONF_SEARCH]).strip()
                )
                ...
                if not restaurants:
                    ...
                    self._restaurants = {
                        restaurant.page_url: restaurant for restaurant in restaurants
                    }
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(CONF_SEARCH): selector.TextSelector(
    ...
    async def async_step_restaurant(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        errors: dict[str, str] = {}
        if user_input is not None:
            ...
            try:
                ...
                restaurant_id = restaurant_id_from_url(resolved.page_url)
            except CannotConnect:
                errors["base"] = "cannot_connect"
            except RestaurantNotFound:
                errors["base"] = "cannot_parse"
            else:
                # The fiche entry id is stable.  The Rxxxxx code is only the
                # current menu endpoint and can rotate over time.
                ...
                return self.async_create_entry(
                    ...
                    data={
                        ...
                        CONF_ENTITY_PREFIX: restaurant_id,
                        CONF_RESTAURANT_CODE: resolved.code,
                        CONF_RESTAURANT_NAME: resolved.name,
                        CONF_RESTAURANT_URL: resolved.page_url,
                        CONF_RESTAURANT_CITY: resolved.city,
                        CONF_RESTAURANT_POSTAL_CODE: resolved.postal_code,
                    },
                )
        options = [
            selector.SelectOptionDict(value=url, label=restaurant.label)
            for url, restaurant in self._restaurants.items()
        ]
        return self.async_show_form(
            step_id="restaurant",
            data_schema=vol.Schema(
                {
                    vol.Required(CONF_RESTAURANT): selector.SelectSelector(
